...
参考信息：
- 今天日期（曼谷）：__TODAY_BKK__
- 当前所在 ISO 周：__ISO_WEK__
- 已知艺人：jane (janeeeyeh)、kao (kaosupassara9)
- 艺人昵称：jane 也可能被叫作 "小叶"、"芷妤"、"叶芷妤"；kao 也可能叫 "小九"、"雪碧"、"九妹"
...
def deepseek_parse(text: str) -> dict:
    ...
    # Manual substitution — the prompt body contains JSON braces that .format() would choke on
    system = (
        INTENT_PROMPT
        .replace("__TODAY_BKK__", today_bkk)
        .replace("__ISO_WEK__", f"{iso_year}-W{iso_week:02d}")
    )
